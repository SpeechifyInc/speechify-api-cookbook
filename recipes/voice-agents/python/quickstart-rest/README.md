# Voice Agents: Python quickstart (native REST)
...
> calls the API directly with `requests` and carries the `-rest` suffix. When an SDK adds
...
conversation }`. The `url` is a **LiveKit** room (`wss://…livekit.cloud`); join it with a
  [LiveKit client](https://docs.livekit.io/home/client/) using `token` + `room` to stream
  audio in/out.
